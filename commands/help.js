...
module.exports = {
  ...
  execute: async (api, event, args, commands, prefix, admins, appState, sendMessage) => {
    ...
    // Load all commands and event commands
    ...
    // If a specific command is provided (not a page number)
    ...
    if (args.length > 0 && isNaN(parseInt(args[0]))) {
      ...
      try {
        ...
        // Construct detailed help message for specific command with new design
        ...
        detailedHelp += "🛠️ 『 𝗨𝘀𝗮𝗴𝗲 』\n";

        detailedHelp += `${command.usage || `${prefix}${command.name}`}\n\n`;

        detailedHelp += `> 𝗧𝗵𝗮𝗻𝗸 𝘆𝗼𝘂 𝗳𝗼𝗿 𝘂𝘀𝗶𝗻�_g 𝗖𝗶𝗱 𝗞𝗮𝗴𝗲𝗻𝗼𝘂 𝗯𝗼𝘁! 💖\n`;
    ...
    // Construct the help message with pagination
    ...
    // Add event commands (if any) on the first page only
    ...
    // Add pagination info and footer
