...
module.exports = {
  ...
  async run({ api, event }) {
    ...
    // Load config.json each time the command runs
    ...
    // Ensure senderID is treated as a string to match config.json UIDs
    ...
    if (!subCommand || subCommand === "list") {
      ...
      const getUserNames = async (uids) => {
        ...
        // Fetch names one UID at a time to avoid API issues
      ...
      // Simplified design without bolding or boxes
    ...
    if (subCommand === "add") {
      ...
      if (messageReply) {
        ...
        role = parseInt(args[1]) || 1; // Defaults to 1 (admin) if no role specified
      ...
      // Update config.json with the new lists
